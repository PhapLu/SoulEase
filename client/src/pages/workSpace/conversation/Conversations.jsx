...
export default function Conversations() {
    ...
    return (
        <div className='ws-messages'>
            ...
            <div className={`ws-panels ${selectedConversation ? 'has-chat' : 'no-chat'}`}>
                <aside className='ws-threads'>
                    ...
                    <ul className='ws-thread-list'>
                        {conversations.map((item) => (
                            <li key={item.id} className={`ws-thread ${selectedConversation?.id === item.id ? 'active' : ''}`} onClick={() => setSelectedConversation(item)}>
                                ...
                                <div className='ws-thread__time'>
                                    ...
                                    {item.unread && <span className='ws-dot' aria-hidden='true' />}
                                </div>
                            </li>
                        ))}
                    </ul>
                ...
                {selectedConversation ? (
                    <section className='ws-chat'>
                        <header className='ws-chat__header'>
                            ...
                            <div>
                                <div className='ws-chat__name'>{selectedConversation.name}</div>
                ...
                <aside className='ws-assistant'>
                    <div className='ws-assistant__header'>
                        <img src={Soulra} alt='Soulra AI icon' width={40} height={40} style={{ display: 'block' }} />
                        <div>
                    ...
                    <div className='ws-assistant__body'>
                        {assistantMessages.map((msg) => (
                            <div key={msg.id} className={`ws-assistant__bubble ${msg.sender === 'assistant' ? 'ws-assistant__bubble--reply' : ''}`}>
                                {msg.sender === 'assistant' && <span className='ws-chip'>S</span>}
                                <p>{msg.text}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <form className='ws-assistant__input' onSubmit={(e) => e.preventDefault()}>
                        <input type='text' placeholder='Ask any request...' value={assistantText} onChange={(e) => setAssistantText(e.target.value)} />
                        <button type='submit'>
                            <FontAwesomeIcon icon={faPaperPlane} />
                        </button>
                    </form>
