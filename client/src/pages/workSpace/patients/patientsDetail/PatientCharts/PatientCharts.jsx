...
export default function PatientCharts({ patientIdProp }) {
    ...
    // --- Render ---
    if (loading) return <div className="charts-section"><p>Loading assessment charts...</p></div>;
    
    if (errorMsg) return <div className="charts-section"><EmptyChart message={errorMsg} /></div>;

    const { history, latest } = chartData;

    // Nếu không có dữ liệu history
    if (!history || history.length === 0) {
        return <div className="charts-section"><EmptyChart message="No assessment records (PHQ-9/GAD-7) found in treatment sessions." /></div>;
    }

    // Data cho biểu đồ Risk
    const riskBarData = ["Low", "Medium", "High"].map((r) => ({
        name: r,
        value: (latest?.risk || "Low") === r ? 1 : 0.2, 
        color: RISK_COLORS[r]
    }));
    ...
    return (
        <div className="charts-section">
            <div className="charts-header">
                ...
                <button className="icon-btn" title="Edit Assessment"><Pencil size={18} /></button>
            </div>
