        ...
        <div className='chart-container'>
            <ResponsiveContainer width='100%' height='100%'>
                <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey='day' axisLine={false} tickLine={false} tick={{ fill: '#333', fontSize: 12 }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fill: '#333', fontSize: 12 }} domain={[0, 100]} ticks={[0, 20, 40, 60, 80, 100]} />
                    <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }} />
                    <Line type='monotone' dataKey={dataKey} stroke={THEME_COLOR} strokeWidth={2} dot={{ r: 6, fill: THEME_COLOR, strokeWidth: 0 }} activeDot={{ r: 8 }} />
                </LineChart>
            </ResponsiveContainer>
        </div>
    </div>
...
export default function PatientCharts({ patientData }) {
    ...
    useEffect(() => {
        ...
        const fetchCharts = async () => {
            try {
                setLoading(true)
                const res = await apiUtils.get(`/patientRecord/readPatientCharts/${patientId}`)
                console.log(res.data.metadata)
                const { weekData, taskData } = res.data.metadata
    ...
    if (!weekData.length || !taskData) return null

    const donutData = [
        { name: 'Completed', value: taskData.completed },
        { name: 'Remaining', value: taskData.remaining },
    ]
            ...
            <div className='charts-grid'>
                ...
                <div className='chart-card'>
                    ...
                    <div className='chart-container donut-container'>
                        <ResponsiveContainer width='100%' height='100%'>
                            <PieChart>
                                <Pie data={taskData} cx='50%' cy='50%' innerRadius={60} outerRadius={80} startAngle={90} endAngle={-270} dataKey='value' stroke='none'>
