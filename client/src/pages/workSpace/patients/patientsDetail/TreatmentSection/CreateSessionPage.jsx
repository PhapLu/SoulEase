...
export default function CreateSessionPage() {
    ...
    const [stageOptions, setStageOptions] = useState(['Stage 1: Stabilization & coping skills'])
    ...
    useEffect(() => {
        ...
        if (folderId && patientRecordId) {
            navigate(`/workspace/patients/folder/${folderId}/${patientRecordId}/treatment`, { replace: true })
        } else {
        ...
        const fetchSymptoms = async () => {
            ...
            try {
                ...
                symptomList.forEach((s) => {
                    const exists = merged.some((m) => m.name?.toLowerCase() === (s.name || '').toLowerCase() && m.sign?.toLowerCase() === (s.sign || '').toLowerCase())
    ...
    const goBackToTreatment = () => {
        if (folderId && patientRecordId) {
            navigate(`/workspace/patients/folder/${folderId}/${patientRecordId}/treatment`)
        } else {
            navigate(-1)
        }
    }
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const prevSessions = Array.isArray(record?.treatmentSessions) ? record.treatmentSessions : Array.isArray(record?.treatmentSections) ? record.treatmentSections : []
            ...
            selectedSymptoms.forEach((s, idx) => {
                const existsIdx = mergedSymptoms.findIndex((m) => (m.name || '').toLowerCase() === (s.name || '').toLowerCase() && (m.sign || '').toLowerCase() === (s.sign || '').toLowerCase())
                if (existsIdx === -1) {
                    mergedSymptoms.push({
                        id: s.id || `sym-${Date.now()}-${idx}`,
            ...
            const sanitizedStage = stageOptions.includes(form.stage) ? form.stage : stageOptions[0]
            ...
            const nextSessions = [newSession, ...prevSessions].sort((a, b) => (b.date || '').localeCompare(a.date || ''))
            ...
            setErr(e2?.response?.data?.message || e2?.message || 'Create session failed')
        } finally {
            setSaving(false)
        }
    }
                ...
                <form className='tp-form' onSubmit={onSubmit}>
                    <div className='tp-form__grid'>
                        <label className='tp-form__field'>
                            ...
                            <input className='tp-input' type='date' value={form.date} disabled readOnly />
                        </label>
                                ...
                                <div style={{ display: 'grid', gap: 10 }}>
                                    {form.symptoms?.length ? (
                                        form.symptoms.map((sym, idx) => {
                                            const match = sym.name ? symptomsOptions.find((opt) => opt.name?.toLowerCase() === sym.name.toLowerCase()) : null
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <label className='tp-form__field'>
                                                            <div className='tp-form__label'>Name</div>
                                                            <select
                                                                className='tp-input'
                                                                value={selectValue}
                                                                onChange={(e) => {
                                                        ...
                                                        <label className='tp-form__field'>
                                                            <div className='tp-form__label'>Sign</div>
                                                            <input
                                                                className='tp-input'
                                                        ...
                                                        <div className='tp-actions' style={{ justifyContent: 'flex-start' }}>
                                                            <button
                                                                type='button'
                                        ...
                                        <span className='tp-muted'>No symptoms selected.</span>
                                    )}
                                </div>
                    ...
                    <div className='tp-form__actions'>
                        <button type='button' className='tp-btn tp-btn--ghost' onClick={goBackToTreatment}>
                            Cancel
                        </button>
