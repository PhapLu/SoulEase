        ...
        <section className='pd-header'>
            ...
            <div className='pd-info'>
                <div className='pd-info__row'>
                    <h2>{isEditing ? <input className='pd-input pd-input--title' value={editForm?.fullName || ''} onChange={(e) => onFieldChange('fullName', e.target.value)} /> : patient?.fullName || 'Patient Name'}</h2>

                    <div className='folder-header-actions-row'>
                        {isEditing ? (
                            <>
                                <button className='folder-save-btn' onClick={onSaveEdit} disabled={saving}>
                                    {saving ? 'Saving...' : 'Save'}
                                </button>
                                <button className='folder-cancel-btn' onClick={onCancelEdit} disabled={saving}>
                                    Cancel
                                </button>
                            </>
                        ) : (
                ...
                <div className='pd-info__grid'>
                    ...
                    <InfoField label='Address' isEditing={isEditing} type='text' value={editForm?.address} display={patient?.address} onChange={(v) => onFieldChange('address', v)} />

                    {/* Gender riêng vì dùng <select> */}
                    <div className='pd-field'>
                        <span className='pd-label'>Gender:</span>
                        {isEditing ? (
                            <select className='pd-input' value={editForm?.gender || ''} onChange={(e) => onFieldChange('gender', e.target.value)}>
                            ...
                            <span>{patient?.gender ? patient.gender[0].toUpperCase() + patient.gender.slice(1) : 'N/A'}</span>
                        )}
                    </div>
                </div>
            </div>
