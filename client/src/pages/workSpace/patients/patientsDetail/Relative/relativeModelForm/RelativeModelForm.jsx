...
export default function RelativeModalForm({ onClose, onSubmit }) {
    ...
    const relationshipOptions = useMemo(() => ['Family', 'Spouse', 'Parent', 'Child', 'Sibling', 'Caregiver', 'Friend', 'Other'], [])
    ...
    const validate = () => {
        ...
        if (phone && phone.length !== 10) next.phoneNumber = 'Phone must be 10 digits.'
    ...
    const handleSubmit = (e) => {
        ...
        const relationshipFinal = formData.relationship === 'Other' ? (formData.relationshipOther || '').trim() : formData.relationship
    ...
    return (
        <div className='relative-modal-overlay' onClick={onClose}>
            <div className='relative-modal' onClick={(e) => e.stopPropagation()}>
                <button className='relative-modal-close' onClick={onClose} type='button'>
                    ✕
                </button>
                ...
                <form className='relative-form' onSubmit={handleSubmit}>
                    ...
                    <div className='form-group'>
                        ...
                        <div className='input-with-icon'>
                            <input type='tel' name='phoneNumber' className='form-input' placeholder='10 digits' value={formData.phoneNumber} onChange={handleChange} inputMode='numeric' pattern='\d{10}' maxLength={10} />
                        </div>
                    ...
                    <div className='form-group'>
                        ...
                        {formData.relationship === 'Other' && (
                            <>
                                <div style={{ height: 8 }} />
                                <div className='input-with-icon'>
                                    <input type='text' name='relationshipOther' className='form-input' placeholder='Specify relationship' value={formData.relationshipOther} onChange={handleChange} required />
                                </div>
                                {errors.relationshipOther && <div className='form-error'>{errors.relationshipOther}</div>}
                            </>
                        )}
                    </div>
