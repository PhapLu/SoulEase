...
const systemNotifications = [
    {
        ...
        message: 'You have successfully created a new client in Folder 1. You can now start sharing documents and messages with this client.',
    ...
    },
    {
        ...
        message: 'Your SoulEase Plus plan will expire in 3 days. Renew now to keep encrypted storage and priority support.',
    ...
    },
    {
        ...
        message: 'We could not process your last payment. Please update your billing information to avoid interruption.',
...
const mockMessages = [
    ...
    },
    {
        ...
        content: 'I just shared a new document in my folder. Please take a look when you can.',
...
export default function Notifications() {
    ...
    const unreadLast3h = useMemo(() => getUnreadMessagesLast3Hours(messages), [messages])
    ...
    const allNotifications = useMemo(() => {
        ...
        return [...messageNotifications, ...sys].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        ...
        <div className='notif-page'>
            ...
            <section className='notif-card'>
                <div className='notif-page-header'>
                    <h2>Notifications</h2>
                    <p className='notif-page-subtitle'>System updates & unread patient messages in the last 3 hours.</p>
                </div>

                <div className='notif-list'>
                    {allNotifications.map((n) => (
                        <button key={n.id} type='button' className='notif-item' onClick={() => setSelected(n)}>
                            <div className={`notif-dot notif-dot-${n.type === 'success' ? 'success' : n.type === 'error' ? 'error' : n.type === 'warning' ? 'warning' : 'info'}`} />

                            <div className='notif-item-main'>
                                <div className='notif-item-title-row'>
                                    <span className='notif-item-title'>{n.title}</span>
                                    {n.category && <span className='notif-item-category-chip'>{n.category}</span>}
                                </div>

                                <div className='notif-item-message'>{n.message.length > 90 ? `${n.message.slice(0, 90)}…` : n.message}</div>
                            </div>
                    ...
                    {allNotifications.length === 0 && <div className='notif-empty'>No notifications at the moment 🎉</div>}
                </div>

                {/* Detailed Alert Popup */}
                <NotificationPopup open={!!selected} title={selected?.title} message={selected?.message} type={selected?.type || 'info'} category={selected?.category} from={selected?.from} createdAt={selected?.createdAt} onClose={() => setSelected(null)} autoCloseMs={6000} />
            </section>
        </div>
    )
}
