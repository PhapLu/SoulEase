...
const LANG_OPTIONS = ['English', 'Vietnamese', 'Spanish', 'French', 'Chinese', 'Other']
...
export default function DoctorDetail() {
    ...
    useEffect(() => {
        const fetchDoctorDetail = async () => {
            try {
                ...
                const strippedName = rawName.startsWith('Dr. ') ? rawName.slice(4) : rawName
    ...
    const handleFieldChange = (field, value) => {
        ...
        if (field === 'phone') {
            ...
            setErrors((prev) => {
                const copy = { ...prev }
                if (!/^\d{10}$/.test(digits)) copy.phone = 'Phone must be exactly 10 digits.'
        ...
        if (field === 'experience') {
            ...
            setErrors((prev) => {
                const copy = { ...prev }
                if (filtered !== '' && (!Number.isFinite(Number(filtered)) || Number(filtered) < 0)) copy.experience = 'Experience must be a non-negative number.'
    ...
    const displayName = `Dr. ${String(editForm?.fullName || doctor?.fullName || '').trim()}`
                ...
                <section className='dd-header'>
                    ...
                    <div className='dd-info'>
                        <div className='dd-info__row'>
                            <h2>{isEditing ? <input className='pd-input pd-input--title' value={editForm?.fullName || ''} onChange={(e) => handleNameChange(e.target.value)} placeholder='Full name' /> : displayName}</h2>

                            <div className='folder-header-actions-row'>
                                {isEditing ? (
                                    <>
                                        <button className='folder-save-btn' onClick={handleSave} disabled={saving || Object.keys(errors).length > 0}>
                                            {saving ? 'Saving...' : 'Save'}
                                        </button>
                                        <button className='folder-cancel-btn' onClick={handleCancel} disabled={saving}>
                                            Cancel
                                        </button>
                                    </>
                                ) : (
                        ...
                        <div className='dd-info__grid'>
                            ...
                            <div className='dd-field'>
                                ...
                                {isEditing ? <input className='pd-input' value={editForm?.email || ''} onChange={(e) => handleFieldChange('email', e.target.value)} /> : <a href={`mailto:${doctor?.email}`}>{doctor?.email}</a>}
                            </div>
                            ...
                            <div className='dd-field'>
                                ...
                                {isEditing ? (
                                    <div style={{ width: '100%' }}>
                                        <input className='pd-input' value={editForm?.phone || ''} onChange={(e) => handleFieldChange('phone', e.target.value)} />
                                        {errors.phone && (
                                            <div
                                                style={{
                                                    color: '#b82020',
                            ...
                            <div className='dd-field'>
                                ...
                                {isEditing ? <input className='pd-input' type='date' value={editForm?.birthday || ''} onChange={(e) => handleFieldChange('birthday', e.target.value)} /> : <span>{doctor?.birthday || 'N/A'}</span>}
                            </div>

                            {/* Age (computed) */}
                            <div className='dd-field'>
                                <span className='dd-label'>Age:</span>
                                {isEditing ? <input className='pd-input' type='number' value={editForm?.age !== undefined ? editForm?.age : ''} onChange={(e) => handleFieldChange('age', e.target.value)} min={0} /> : <span>{doctor?.age ?? editForm?.age ?? 'N/A'}</span>}
                            </div>

                            {/* Address */}
                            <div className='dd-field'>
                                <span className='dd-label'>Address:</span>
                                {isEditing ? <input className='pd-input' value={editForm?.address || ''} onChange={(e) => handleFieldChange('address', e.target.value)} /> : <span>{doctor?.address || 'N/A'}</span>}
                            </div>

                            {/* Experience (years) */}
                            <div className='dd-field'>
                                <span className='dd-label'>Experience (years):</span>
                                {isEditing ? (
                                    <div style={{ width: '100%' }}>
                                        <input className='pd-input' type='number' min={0} value={editForm?.experience ?? ''} onChange={(e) => handleFieldChange('experience', e.target.value)} />
                                        {errors.experience && (
                                            <div
                                                style={{
                                                    color: '#b82020',
                            ...
                            <div className='dd-field'>
                                ...
                                {isEditing ? (
                                    <div style={{ width: '100%' }}>
                                        <select className='pd-input' multiple value={editForm?.language || []} onChange={(e) => handleLanguageChange(e.target.selectedOptions)} style={{ height: 110 }}>
                                    ...
                                    <span>{Array.isArray(doctor?.language) ? doctor.language.join(', ') : doctor?.language || 'N/A'}</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className='dd-speciality'>
                            ...
                            {isEditing ? <input className='pd-input' value={editForm?.speciality || ''} onChange={(e) => handleFieldChange('speciality', e.target.value)} /> : <span>{doctor?.speciality || 'N/A'}</span>}
                        </div>
                    </div>
                </section>
            </div>
