 ...
 */
export function generateReferralCode(userId, secret = process.env.REFCODE_SECRET, length = 12) {
    ...
    return raw.length >= length ? raw.slice(0, length) : raw.padStart(length, '0')
}
...
export const RESERVED_PATHS = new Set(['', 'posts', 'archive', 'not-found', 'order-management', 'my-commission-requests', 'talent-hub', 'badges', 'orders', 'characters', 'commission-hub', 'oc-universe', 'galaxies', 'newss', 'search', 'threads', 'paypal', 'dashboard', 'auth', 'statics', 'error', 'forbidden', 'favicon.ico', 'robots.txt', 'sitemap.xml'].map((s) => s.toLowerCase()))
